...
function addItem(): void {
    ...
    const quantidade = parseInt(prompt("Digite a quantidade: ") || "0");
...
function removeItem(): void {
    ...
    let indiceItem = shoppingList.findIndex(item => item.nome === removerItem);

    if (indiceItem !== -1) {
        let confirmacao = confirm(`Tem certeza que deseja remover o item ${removerItem}?`);
        
        if (confirmacao) {
            shoppingList.splice(indiceItem, 1);
            alert("Item removido!");
        } else {
...
function editList(): void {
    ...
    let indiceItem = shoppingList.findIndex(item => item.nome === Editaritem);

    if (indiceItem !== -1) {
        let item = shoppingList[indiceItem];
...
function displayList(ordenarPor?: 'nome' | 'categoria' | 'quantidade', filtrarPorCategoria?: string, filtrarPorStatus?: string): void {
    let itensFiltrados = [...shoppingList];
...
function markItem(): void {
    ...
    let indiceItem = shoppingList.findIndex(item => item.nome === itemMarcar);
...
function resumeItem(): void {
    let totalItens = shoppingList.length;
